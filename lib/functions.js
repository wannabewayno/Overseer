...
Fn.switch = answer => Fn[answer](); //a callback switch, directs users' answer to the associated callback
    ...
    Fn.showDashboard = () => {
        ...
        // ------------------------ Display chosen Team funstion -------f---------------
        Fn.displayTeam = team => {
            //destructure team data and put all teams in an array to be processed into the render function.
           const employeeList = destructure(data[team]); 
            ...
            fs.writeFile(outputPath,html,err=>{
                ...
                const startServer = require('../output/server.js')
            });
    ...
    Fn.viewTeam = () => {
        ...
        Fn.employeeOptions = answer => {
            if (answer === "chooseEditType"){
                temp.currentDirectory = temp.currentDirectory.slice(0,temp.currentDirectory.indexOf(':'))+':';
                Q.queue.push(Q.chooseEditType);
            } else {
                temp.currentDirectory += ` ${answer.getRole()}: ${answer.getName()}:`;
                temp.data.push(answer);
        ...
        Fn.changeProperty = answer => {
            ...
            temp.data = [employee];
            if(answer==='name'){
                Fn.updateEmployeePath()
                // const oldKey = employee.name;
                // const newKey = answer;
                // Fn.getEmployeePath()[newKey] = Fn.getEmployeePath()[oldKey];
                // delete Fn.getEmployeePath()[oldKey];
            } 
...
Fn.updateEmployeePath = newKey =>{
    ...
    const oldKey = temp.currentDirectory.slice(secondColonIndex+2,-1);
    Fn.getEmployees()[role][newKey] = Fn.getEmployees()[role][oldKey];
    delete Fn.getEmployees()[role][oldKey];
    temp.currentDirectory = `${temp.currentDirectory.slice(0,firstColonIndex)}: ${role}: ${newKey}:`;
}
